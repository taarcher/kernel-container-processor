...
int processor_container_delete(struct processor_container_cmd __user *user_cmd){
	...
	iterC->count -= 1;
    printk( KERN_DEBUG "decrement counter for container %d to %d", iterC->cid, iterC->count );
    //check for propper repositioning of the pointers
    printk( KERN_DEBUG "check if Con %d count is 0(%d)", iterC->cid, iterC->count );
    if(iterC->count > 0) {
        printk( KERN_DEBUG "check if end of list" );
        if(temp->next != NULL) {
            printk( KERN_DEBUG "end of list, reset pointers");
            temp->next->prev = temp->prev;
        }
        printk( KERN_DEBUG "set next pointer" );
...
int processor_container_switch(struct processor_container_cmd __user *user_cmd)
{
    ...
    printk( KERN_DEBUG "search for active task" );
	...
	while( temp == NULL ){
        ...
        if( temp == NULL && iterC->next == NULL){
            printk( KERN_DEBUG "thread wasn't found." );
            mutex_unlock( &mtx );
            return 0;
        }
	}
    
    printk( KERN_DEBUG " check for multiple threads in container" );
    if( iterC->count > 1 ){
        printk( KERN_DEBUG "%d threads in container", iterC->count );
        printk( KERN_DEBUG "check if end of list" );
        if ( temp->next == NULL ){
            iterT = iterC->start->next;
            printk( KERN_DEBUG "end of list, move to top" );
        } else {
        printk( KERN_DEBUG "not last, so set the next" );
        iterT = temp->next;
        }
        printk( KERN_DEBUG "set %d to sleep", ctask->pid );
        //wake up / put to sleep
        set_current_state( TASK_INTERRUPTIBLE );
        printk( KERN_DEBUG "wake up %d", iterT->task->pid );
